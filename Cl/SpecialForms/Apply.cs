...
public class Apply(ClSymbol symbol, ClCallable car, ClObj cdr) : ClCell(car, cdr)
{
    public override IContext Reduce(IContext ctx)
    {
        ...
        if (Car is NativeFn nativeFn)
        {
            ...
            args = ClSymbol.Read.Equals(symbol) && ctx is Context c ? args.Append(c) : args;
            var result = nativeFn.Apply([.. args]);
